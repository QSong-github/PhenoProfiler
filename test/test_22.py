...
def whitening_transform(meta, features, config):
    ...
    # 获取控制组数据并转换为NumPy数组
    control_mask = wells["Treatment"] == "DMSO@0"  # change
...
def main():
    ...
    if not os.path.exists(CONFIG["save_path"] + "PhenoProfiler_alltrain_22test.npy"):
        ...
        np.save(CONFIG["save_path"] + "PhenoProfiler_alltrain_22test.npy", img_embeddings.T)
